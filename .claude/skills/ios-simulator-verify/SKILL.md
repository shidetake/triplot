...
作ったものだけで、staging DB なら直接 SQL で消してよい。本番 DB のデータは原則
書き換えない・消さないので、そもそも本番で検証データを作らない。どうしても本番の
データに触る必要がある時だけ、理由を説明してユーザーの許可を得る（AGENTS.md
「本番のデータは原則書き換えない・消さない」）。
