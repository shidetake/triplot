...
function estimateListContentH(places: PlaceRow[]): number {
  return (
    LIST_HEADER_H +
    LIST_BOTTOM_PADDING +
    places.reduce((h, p) => h + LIST_ROW_H + (p.note ? LIST_ROW_NOTE_H : 0), 0)
  );
}
...
function SavedPlaceRow({
  ...
  const rowFocusStyle = useAnimatedStyle(() => {
    ...
    return {
      opacity:
        1 - focusActive.value * (1 - closeness) * (1 - FOCUS_DIM_OPACITY),
...
export default function PlacesTab() {
  ...
  const pendingCenterScrollTimerRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const pickerCenteringEnabled =
    filteredPlaces.length >= PICKER_CENTERING_MIN_ITEMS;
  ...
  const goToMyLocation = async () => {
    ...
    if (!coord) {
      ...
      if (perm.status !== "granted") {
        Alert.alert(t("locationUnavailable"), t("locationPermissionBody"));
        return;
      }
      try {
          ...
          >
            ...
            {pinDraft && (
              ...
              <Marker
                ...
                anchor={{ x: 0.5, y: 0.9 }}
                // 他の全マーカーが zIndex を明示している以上、これも揃える
                // （一部だけ未指定だと重なり順が不安定になる）。ドラッグ中の
                // 仮ピンは編集中の赤ピンと同格で常に最前面でよい。
                zIndex={200}
                draggable
                onDragEnd={(e) => {
      ...
      {listOpen && (
        <ScreenStackItem
          ...
          sheetInitialDetentIndex={
            listMode === "search" ? 1 : editing ? 0 : 0 // browse も開いた瞬間は小さい方（画面の半分程度）から見せる
          }
              ...
              ListHeaderComponent={
                <View style={styles.sheetHeader}>
                  ...
                  {/* 検索結果の時だけ × を出す（本家 Google マップと同じ）。
                      シートには普通 × を置かない（下げれば閉じると分かるため）が、
                      ここは「下げる」＝結果を残したまま畳むなので、検索そのものを
              ...
              ListHeaderComponent={
                <>
                  ...
                  {/* ピッカー中の先頭/末尾の行を一覧の中央まで持ってこられる
                    ...
                    ずれて見えてしまうため）。 */}
                  {pickerTopPad > 0 && (
                    <View style={{ height: pickerTopPad }} />
                  )}
                </>
              }
        ...
        >
          <ScrollView contentContainerStyle={styles.formScroll}>
            ...
            <Pressable
              ...
              style={[
                styles.priorityRow,
                !placeFilter && styles.priorityRowSelected,
              ]}
            >
              <Text
                style={[
