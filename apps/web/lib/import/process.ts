...
// 予算は関数の寿命そのものではなく、**1件ぶんの余裕を引いた値**にする:
//
//   関数の寿命（maxDuration）      60s
//   1件の抽出に見込む最悪の時間    30s（実測 15〜30s。LLM 2パス＋場所解決）
//   → 新しい1件を始めてよいのは    25s まで（25 + 30 = 55 < 60）
//
// ここを寿命いっぱいに取ると、締切ぎりぎりで始めた1件が**抽出の途中で関数ごと
// 殺される**（LLM の料金だけ払って結果が入らない）。以前の「1回25件まで」は
// 25 × 15s = 375s で寿命を大きく超えるので、実質この殺され方が上限だった。
//
// 毎分の cron より短いので、実行同士が重なって同じ行を二重に処理することも無い。
