...
export type StoredReceipt = Receipt & {
  ...
  // **この date/time を供給したメールの送信時刻。** 決済通知を現地の壁時計に
  // 直すのに要る（settlementTiming.ts）。合体すると日付は片方のメールから来る
  // ので、「今処理しているメール」の送信時刻とは限らない。日付と一緒に持ち回る。
  //
  // これが無いと、場所が後から解決した時に時刻を取り戻せない（実データ:
  // 単体で処理した時は店名が途中で切れていて場所が解決できず現地化できず、
  // 確定通知が合体した時に店名が揃って場所は解決したが、その時に手元にある
  // 送信時刻は確定通知のもので、日付〔利用通知の 5/1〕と噛み合わず拒否された）。
  sentAt?: string | null;
  // **この date/time がどの土地の壁時計で書かれているか**（IANA）。
  //
  // レシートの日時はアプリの外から来る「実際に起きた瞬間」で、銀行は自国の暦で
  // 書き、店は店の壁時計で書く。**壁時計だけを持つと、その値がどの暦のものか
  // が分からない。** 現地化（settlementTiming.ts）はこの2つのフィールドを
  // 書き換えるので、書き換えた後の値と書き換える前の値が見分けられず、同じ
  // 処理をもう一度通すとさらにずれる。
  //
  // 実データ: ソニー銀行の「ご利用のお知らせ」（日本の暦で 5/2、時刻なし）が
  // 送信時刻からホノルルの 5/1 18:02 に直った後、「金額確定のお知らせ」と
  // 合体した際にもう一度現地化され、5/1 18:02 を日本時間として読み直して
  // 4/30 23:02 になった。19時間＝日本とハワイの時差ぶん余計にずれている。
  //
...
function localizedReceipt(
  ...
  // **すでにどの土地の壁時計で書かれているか分かっているなら、それが答え。**
