...
export const receiptSchema = z.object({
  ...
  // 合体後の金額を機械的に決めるための印（足し算を合体時に推測させない）。
  // 判定するのは「このメールの金額が何か」だけで、他のメールと見比べる判断では
  // ないので迷わない。使い方と、これが要る理由は receiptTotal.ts。
  totalIsDelta: z
    .boolean()
    .describe(
      "total が、既に届いているはずの取引に**後から足される分**（差額調整・" +
        "確定による更新・後追いのチップ）なら true。その取引の総額なら false。" +
        "本文が『差額』『調整』『更新』を指していて、金額が会計全体としては" +
        "不自然に小さいなら true。単独のレシートは常に false",
    ),
