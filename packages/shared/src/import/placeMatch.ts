...
//
// **これは Google で引けなかった時の受け皿。** 下書きの場所の事前入力は
// Google で解決できた場所を優先し、ここはその後ろに置く（drafts.ts）。
//
// 名前の照合は、広い名前が具体的な店を飲み込む方向に外れる。実データ:
// ハナウマ湾からの帰りの行き先として保存された地名「Waikiki」が、部分一致で
// 「Hula Grill Waikiki」に一致し（1語が全部含まれる＝0.7）、Google が正しく
// 解決していたレストランを上書きしていた。ワイキキで買えば店名に Waikiki が
// 入るので、当たる範囲が広い。
//
// 同じ訪問から出た複数のメールを1つの場所に寄せる、という当初の目的は、
// Google で引けている限り場所 ID 側で達成される（find_or_create_trip_place が
// 同じ Google の場所 ID の行を再利用する）。実データでも費用の下書き45件中
// 42件は Google で解決できていた。名前の照合が本当に要るのは、カードの明細
// 表記（"WHOLEFDS QUE#10615" 等）で店として引けない残りだけ。
...
export function nameTokens(s: string): string[] {
  ...
  return stripPaymentPrefix(s)
    .toLowerCase()
    // **ラテン文字のアクセント記号を落とす。** カード明細は ASCII なので、
    // 現地表記と1文字も一致しないことがある（実データ: ハワイの
    // "Lēʻahi Market" と明細の "LEAHI MKT BAR" が一致度 0.0 で、場所が
    // 解決できず時刻も出なかった）。分解してから U+0300–U+036F だけ消す
    // ——**日本語の濁点（U+3099/U+309A）はこの範囲の外**なので「が」が「か」に
    // ならない。**落とした後に必ず合成し直す**——分解したままだと「が」が
    // 「か」＋結合文字に割れたままで、下の「記号→空白」で分断される。
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .normalize("NFC")
    // アポストロフィ類（ハワイ語のオキナ ʻ を含む）は**消して繋げる**。空白に
    // すると "lēʻahi" が "le" と "ahi" に割れて、やはり一致しない。
    .replace(/[\u0027\u02bb\u02bc\u2018\u2019]/g, "")
