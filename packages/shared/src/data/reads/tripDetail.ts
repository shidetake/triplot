...
export async function fetchTripDetailRows(sb: DB, tripId: string) {
  const [
    ...
    sb
      .from("events")
      .select(
        "id, title, kind, all_day, start_at, end_at, start_tz, end_tz, tz_disambig_transit_id, tz_disambig_side, start_place_id, end_place_id, visibility, note, created_by_member_id, created_at, participants_everyone, event_participants(member_id)",
      )
      .eq("trip_id", tripId)
