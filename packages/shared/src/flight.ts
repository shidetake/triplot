 ...
 * 予測の元にする日を運航日一覧から選ぶ。
 *
 * **年の中で対象日に一番近い日**（年をまたいで測る）。同率なら新しい方。
 *
 * 「直近の日」ではなく「時期の近い日」なのは、時刻表が夏ダイヤ/冬ダイヤで
 * 組み替わるため。ただし ZG002 の15ヶ月分を実測した限り、**出発時刻はほぼ
 * 常に 19:10 で不動**（1日だけ 19:05）、到着は 07:05〜07:50 に散り、その散り方は
 * ダイヤ期間と対応していなかった。そのため IATA のダイヤ期間で先に振り分ける
 * ような細工はしない（手元の実測が支持しない理屈でロジックを増やさない）。
 * どの日を選んでも出発時刻はまず当たり、到着は所要時間ぶんの誤差に収まる。
 */
