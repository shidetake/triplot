...
**面や枠を持つカードは詰めてよい。**カード自身が境界を持っているので、隙間は
「隣と別物」を示す最小限で足りる。境界を持たない素の行（テキストだけ）の時に
初めて、間隔か仕切り線（`divide-y divide-foreground/10`）が要る。

**まとまりの境目は絶対値ではなく比で効く。**中を12にすると境目は24では足りず
36〜48が要る。画面の縦は有限なので、中を詰めるほど境目が少ないコストで
はっきりする（中8・境目24＝3倍で十分）。

例（受信箱）: 転送先アドレス／取り込み待ち（待つだけ）／未確定の下書き
（旅行を割り当てる・破棄する）／使用量、の4つが別のまとまり。各まとまりの中は
8px、境目は 24px ＋ 見出し。
