...
**ボタンの大きさは iOS が決めるので、行の高さに合わせようとしない。** iOS 26 のボタンは
行いっぱいの帯ではなく**中に浮かぶ丸いボタン**で、「行の高さ − 4pt、ただし 50pt まで」で
描かれる。行が高ければ中央に置かれる。小さくする手立ては無い（`controlSize`・
`imageScale` はどちらも効かない＝大きさを決めているのは SwiftUI の modifier ではない）。

**行の最小の高さは 48pt。** これがこの仕組みで詰められる下限で、実測で 48pt の行には
ボタン（44pt）が丸ごと収まり、44pt では下が平らに切れた。**器ではなく行の側に与える** —
