...
- **iOS はスワイプ**（`react-native-gesture-handler` の `Swipeable`）。常時表示のゴミ箱は置かない
  （全行に赤が並ぶと、赤が状態ではなく背景になる）
- **web は行のゴミ箱**。マウスにスワイプは無く、hover で出すと「触れる要素が見えない」
- ここは**揃えない差分**（プラットフォームの流儀）→ [design/platform-parity.md](./design/platform-parity.md)
