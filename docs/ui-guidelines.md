...
行いっぱいの帯ではなく**中に浮かぶ丸いボタン**で、実測 47〜50pt から大きくならない
（行を 300pt にしても変わらない）。行が高ければ中央に置かれる。

**行には最小の高さ（56pt）を与える。** 逆に行が低いとボタンが切られる（実機
フィードバック: 1行の TODO で赤いボタンの下が欠けた。2行の行では収まるので気付き
にくい）。**器ではなく行の側に与える** — 器だけ広げると中身が元の高さのまま上に寄って
下に空きができる。以前は HIG のタップ対象の最小である 44pt にしていたが、ボタン
（47pt）が入り切らないので実測から決め直した。
