...
### OAuth ログインボタン（Google/Apple）も各社ブランドガイドラインに合わせる

`OAuthSignInButton`（`components/oauth-sign-in-button.tsx`）は triplot の共通 `<Button>` を使わず、
native `<button>` に Google/Apple それぞれの公式ブランドガイドラインの配色をそのまま直書きする
（上の地図・Google 連携と同じ「セマンティック色トークン運用の例外」）。Apple の Usage Guidelines は
web にも公式ボタンのデザインガイドライン順守を明記しているため、triplot 内だけで完結する UI ではない。

- 配色はライト/ダークで固定の2パターン（`dark:` variant で切替、JS判定不要）:
  Google＝白地/黒文字＋`#747775`枠（ダークは`#131314`地/`#E3E3E3`文字＋`#8E918F`枠）、
  Apple＝黒地白文字（ダークは白地黒文字）。
- ロゴは `components/oauth-brand-icons.tsx`（`AppleGlyph`/`GoogleGlyph`）。Google の "G" は4色固定で
  ボタン地（ライトの白地・ダークの`#131314`地）にそのまま直接置く（白いチップ等で個別に囲まない
  ＝ Google 公式のダークテーマボタン例示・主要アプリ実例と同じ）。Apple は単色シルエットで
  `currentColor` 任せにし、ボタンの文字色（黒/白）に自動追従させる。
