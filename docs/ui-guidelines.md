...
| | 保存ボタン | 例 |
|---|---|---|
| **1つ選ぶだけで完結する操作** | **置かない**（選んだ時点で保存） | 取り込みの旅行の割り当て・公開範囲・優先度・カテゴリ・スイッチ |
| **複数の入力を1つのまとまりとして確定する** | 置く | 予定フォーム・費用フォーム（途中は不完全・不整合になり得る） |
| **自由入力**（テキスト） | 置く | 表示名・メモ（打っている途中に「決まり」の瞬間が無い） |

**1つ選ぶだけの操作に保存ボタンを置かない。** 選んだ瞬間に意図が確定していて
途中の状態が無いので、ボタンは**ユーザーが既に言ったことをもう一度言わせる**
一手にしかならない。

**即保存にする条件は「結果が見えること」**（[[フィードバック]]）。割り当てなら
行の表示が変わるので足りる。見えないならトーストを出す。

**取り消せない操作はこの話の外。** 保存ボタンではなく確認ダイアログの担当
（[[確認ダイアログ（破壊的操作）]]）。
