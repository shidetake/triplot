...
**ダークモードの濃さは「白地の反転」では作らない**。ライトの 87/60/38%（黒地基準）を
そのまま白基準に反転すると、`muted-foreground`/`subtle-foreground` が前景（ほぼ白）に
近づきすぎて、非活性/非選択と活性/選択の区別がつきにくい（暗い地の上では明るいグレーが
実際のコントラスト比以上に「白に近く」見える知覚効果があり、ライトモードより意図的に
差を広げる必要がある）。実測値（前景 `oklch(0.985)` を基準にした WCAG コントラスト比）:

| トークン | ライト | ダーク（旧: 単純反転） | ダーク（現在） |
|---|---|---|---|
| `muted-foreground` | `oklch(0.510)` | `oklch(0.708)` `#a1a1a1`（前景比 2.48:1） | `oklch(0.55)` `#717171`（前景比 4.65:1） |
| `subtle-foreground` | `oklch(0.699)` | `oklch(0.708)` `#a1a1a1`（muted と同値＝区別不能だった） | `oklch(0.38)` `#424242`（前景比 9.59:1、muted 比 2.06:1） |

タブバーの非選択アイコン・ToggleChip の非選択文字など「非活性/非選択」を示す箇所は
色トークンを直書きせず、必ずこの2トークン（`muted-foreground`/`subtle-foreground`）を使う
＝新しい非活性表現を追加するときも別の薄さを作らない。
