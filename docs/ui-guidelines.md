...
まとめない方がよいものが2つある。どちらも「同種の項目が並んでいる」ように
見えて、実は前提が違う:

- **1項目の内側に開く内訳**。独立した一覧ではなく、その項目の中身なので
  枠の中に枠を作ることになる（例: 受信箱の下書きを開くと出る「合体された
  メール」の内訳）。
- **枠そのものが意味を持つ項目**。まとめると1件に見えて意味が壊れる
  （例: 旅行一覧の「旅行の候補」。破線の枠が「まだ存在しない旅行」を表して
  いるので、複数の候補を1つの破線枠に入れると1つの候補に見える）。

  分かれ目は「その意味が**1件ずつに付いているか、群に付いているか**」。
  群に付いているなら囲いを群の側に移して、中身は普通にまとめてよい
  （例: 旅行画面の「未確定の取り込み」は破線の箱と見出しが群として
  「未確定」を示しているので、中の行は区切り線だけでよく、内側にもう1つ
  枠を作らない）。囲いの
  無い一覧に1件ずつ混ざるとき（旅行の候補）だけ、まとめられない。
