...
それ以外は確認。新しくアンドゥを付けるときは [[元に戻す（アンドゥ）の作り方]] に従う。
...
**引き切ったと見なす距離は行の幅から導く**（画面の広さで手の動きが変わるので、固定の
px だと狭い端末では遠すぎ、広い端末では近すぎる）。半分＝iOS 標準の一覧と同じ感覚。
**越えた瞬間はハプティックで知らせる**（`Haptics.impactAsync` の Medium）。指が面の上に
あって画面が見えていないことがあるので、目だけの合図では足りない。**戻った時にも鳴らす**
——「もう離しても消えない」も同じくらい知りたい報せで、片道だけだと案内にならない。
あわせて**行は指に 1:1 で付いてくる**ようにする（`friction={1}`）——追従を鈍らせると、
行を端まで引いても面が行の半分までしか広がらず、「覆い切ったら消える」という手がかりを
出せない（実測: `friction={2}` では画面端から端まで引いても発動しなかった）。
...
引き切る距離・赤い面の広がり方・ハプティックを一覧ごとに書くと手触りがずれ、覚えた仕草の
価値が無くなる。新しい一覧に付ける時もこれを使う。

**引き切って離したら、その場で畳んで消す。** `Swipeable` は指を離すと「開いた位置」へ
戻すアニメーションを始めるので、そのままだと行が**いったん右へ跳ね返ってから**消え、
消えなかったように見える（実機フィードバック）。跳ね返りは部品の内側の動きで止められない
ので、赤い面と同じ色の板で行を覆ってから畳む（跳ね返りは板の下で起きる）。書き込みの結果は
待たない——消えるはずのものが画面に残っている時間を作らない。
