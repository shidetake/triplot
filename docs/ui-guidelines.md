...
現状アンドゥが有るのは**週カレンダーの予定のドラッグ移動だけ**で、それ以外は確認。
新しくアンドゥを付けるときは [[元に戻す（アンドゥ）の作り方]] に従う。

確認が要るかどうかは、それとは別に**失ったものを取り戻すのにどれだけかかるか**で決める:

| 失うもの | 復旧コスト | 確認 |
|---|---|---|
| TODO | 1行のテキスト。打ち直せば済む | **不要** |
...
**確認を省くなら、一手で消せないようにする。** TODO の削除はスワイプで「削除」を出して
タップ（2手）。**フルスワイプ（一手で消える）は付けない** — あれは取り消せる前提の仕草で、
アンドゥを作ってから解禁する。
