...
意味を作っている線画アイコン）はこの方式の対象外——その場合は無理に塗らず、
色トークンの濃淡（`text-foreground` vs `text-muted-foreground`）だけで選択/非選択を
表す（タブバーの `tabBarActiveTintColor`/`tabBarInactiveTintColor` 等）。
