...
**確認もアンドゥも無いなら、一手で消せないようにする。** スワイプで「削除」を出して
タップ（2手）。**フルスワイプ（一手で消える）はアンドゥとセットで解禁する** — あれは
...
| | iOS | なぜ |
|---|---|---|
| **全行が同じように消せる** | **スワイプ**（`react-native-gesture-handler` の `Swipeable`） | ゴミ箱を常時出すと全行に赤が並び、赤が状態ではなく背景になる。全行が対象なら、一度覚えた仕草がその一覧のどこでも効く |
...
**行には最小の高さ（44pt）を与える。** iOS が描くボタンの方が背の低い行より大きく、
実測値をそのまま渡すと、はみ出したボタンが器で切られる（実機フィードバック: 1行の TODO
〔約32pt〕で赤いボタンの下が切れた。2行の行では収まるので気付きにくい）。**器ではなく行の
側に与える** — 器だけ広げると中身が元の高さのまま上に寄って下に空きができる。44pt は
HIG のタップ対象の最小でもある。
...
（渡すのは消す処理・文言・行の見た目・`measureKey` だけ）。組み方と、RN で描いた行を
