...
- **固定ヘッダーは1ページにつき1本**（`components/app-header.tsx`）。薄い常時表示バー
  （`sticky top-0`・auto-hide しない）で、アプリ内全ページ（`/trips` 配下・`/settings`・`/import`）に出す。
...
- **ヘッダーから開くものはページ遷移でなくオーバーレイ**（受信箱＝`ImportSheetButton`、
  ...
  **閉じれば元の画面に戻る**。以前 受信箱は `/import` へのページ遷移だったが、旅行詳細から
  ...
  に反していた）。`/import` のページ自体はメール内のリンクや直リンクの入口として残す。
- **旅行の操作はアカウントメニューの中の「この旅行」**（広い画面は `TripMenuSection` の
  サブメニュー、狭い画面のシートは `TripMenuRows` の節見出し付き一覧）。1本のバーに
