...
- **1メール = `inbound_emails` の1行**。複数メールが同一取引なら id リンク（`merged_into`）で
  グループ化する（referenceId で辿るのではなく、各メールに id を振って辿る）。
- **抽出と「どの旅行か」の割り当ては LLM の1回の呼び出しで同時に**行う（日付レンジ計算ではなく、
  本文＋候補旅行を一緒に見て推論。日付の取り違えによる誤割り当てを減らす）。
- **確定（費用化）の最終判断は人**（支払者・割り勘・為替レートは機械化しない）。下書き＋レビュー方式。
...
`tripId = null`（受信箱で人が選ぶ）。追加トークンは候補旅行リスト分のみ（別呼び出しはしない）。

## 2. 自動リトライ（失敗そのものはトリガではない）
...
- **BYOK（ユーザ課金）が長期の既定**。早期は運用者課金の AI Gateway を意図的に loss-leader として使う
  （月間取り込み上限でコスト保護）。
- **マージは id リンクで辿る**（referenceId は突き合わせの手がかりであって取得キーではない）。各メールに
  id を振り、`merged_into` で親子をたどる。誤マージは受信箱で「分ける」（unmerge）で戻せる。
