...
- **画面に応じて見た目を変えるときは「何を本質的に切り替えたいか」を見極め、それを決める軸で判定する**。
  ...
  - **使える幅で決まること**（要素が画面に対して大きすぎる等）→ **ビューポート幅**で判定。
    **ルール: 一定幅以上の入力フォームは、狭い画面ではボトムシート（`vaul`）、広い画面ではタップ位置のポップアップ**
    ...
    Base UI にシート部品が無いので vaul を使う（＝下の「ライブラリを使う」段）。
...
- 2（shadcn/Base UI）: メニュー＝Base UI Menu（⋯/アカウント）、ダイアログ＝Base UI Dialog（確認・場所アイコン）、
...
- 4（ライブラリ）: 日付＝`react-day-picker`（Base UI Popover 内）、地図＝Google Maps、
  **狭い画面のフォームのボトムシート＝`vaul`**（Base UI にシート部品が無いため）。
- 5（完全自作）: 週カレンダー・地図上の独自描画・色相ホイール。
- **現状の逸脱（要対応）:** 場所入力のオートコンプリート（`place-picker`/`place-search`）は手書きだが、
  Base UI に **Combobox/Autocomplete がある**ので本来 step2（Base UI を使うべき）。ただし Google Places 連携が絡む
  core flow なので移行は要検証。※旧逸脱（⋯/アカウントメニュー・確認ダイアログ・`CategorySelect`・種別セレクタ）は
  Base UI 化・native radio 化で**解消済み**。
