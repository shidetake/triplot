...
| `text-subtle-foreground` | 60%（→将来 38% に変更可） | プレースホルダ・非活性アイコン |
...
`subtle-foreground` は現在 `muted-foreground` と同値だが、将来アクセシビリティ要件を確認した上で
38% に下げる可能性がある。セマンティックを分けることで補助テキストと独立して変更できる。
