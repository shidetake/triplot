...
## 部品の作り方：native か shadcn か自作か

このプロジェクトは **shadcn プロジェクト**（`components.json` あり・style `base-nova`）だが、shadcn を全部入れず
**チェリーピックする**：`Button` / `Calendar` / `Popover` は実体を `components/ui/` に置き、その他は recipe を
utility/定数で手書き再現する（例: セグメントトラックは ToggleGroup を入れず手書き）。新しい部品を足すときの
**native / shadcn(スタイル版) / 自作(Radix or 手書き)** の選び方:

**まず shadcn 部品を2種類に分けて考える:**

| 種類 | 例 | 中身 | 判断 |
|---|---|---|---|
| **① 素の HTML 要素にクラスを着せただけ** | Button(`<button>`)・Input(`<input>`)・Textarea・Label・Checkbox・Radio | native 要素**そのもの**。styling しても native 挙動（タップ・キーボード・オートフィル・OS ピッカー）を捨てない | **トレードオフ無し→常にスタイル版を使う**（`<Button>` / `<Input>`） |
| **② native を捨てて div+JS で作り直したもの**（Radix） | **Select**・Dropdown・Combobox・Dialog・Tabs・Switch | 見た目の自由と引き換えに **native のモバイル UI を失う** | 下のフローで native か自作かを選ぶ |

**②の判断フロー（モバイルで native にすべきか）:**

1. **その native 要素は“良いモバイル専用 UI”を持つか？**（`<select>`→OS ピッカー〔iOS ドラム/Android シート〕、`type=date`→OS 日付ピッカー、`type=tel`→テンキー）。
2. 持っていて、**それを超える表現が要らない**なら **native のまま**（プレーンな選択肢の `<select>` 等）。
3. **native では出せない中身が要るなら、そこだけ自作**（option にアイコン/2行/アバター、検索付き、複数選択チップ、非同期、期間ハイライト等）。

**このアプリでの帰結（＝実例）:**

- **①は必ずスタイル版**: ボタン→`<Button>`、テキスト入力→`<Input>`（`components/ui/input.tsx`、中身は native `<input>`＋`inputClass`）。checkbox/radio も native 要素のまま（クラスを着せるだけ）。
- **プレーンな選択は native `<select>`**（通貨・支払者・旅行・ステータス）。OS ピッカーがモバイルで最良なので Radix Select は入れない。
- **native で出せない所だけ自作**: `CategorySelect`（option にカテゴリ SVG が要る＝native `<option>` 不可）、`DatePopover`/`DateRangePopover`（旅行期間ハイライト・選択不可レンジが要る）。
- **native 相当が無い物**（⋯メニュー・アカウントメニュー）は「①②の軸」でなくチェリーピック軸で判断＝`menuItemClass`＋`FormPopover` で手書き（Radix DropdownMenu は入れない）。

迷ったら「**native 要素を捨てているか？捨てているなら、その native のモバイル UI より良いものを自分で出す必要が本当にあるか？**」を問う。無いなら native。
...
| **入力・セレクト** | 共通定数 **`inputClass`**（`components/input-class.ts`）= `rounded-md border border-foreground/20 bg-white px-3 py-2 text-sm focus:border-primary focus:outline-none` | レイアウト（`mt-1 block w-full`・`min-w-0`・`flex-1`・`pr-9` 等）は呼び出し側が足す。入力風トリガ（カスタムセレクト）も同じ（`flex items-center ${inputClass}`）。**手書きで枠・bg・focus を並べず必ず `inputClass` を使う**。iOS のフォーカス時ズーム防止は **globals.css がタッチ端末（`any-pointer: coarse`）で 16px を強制**するので入力は `text-sm` のままで可 |
