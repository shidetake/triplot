...
| 役割 | 色 | 例 |
|---|---|---|
| **Primary**（その文脈で最も主要な動作） | **`bg-primary text-primary-foreground`**（hover `bg-primary/90`） | 保存 / 追加 / 編集（その popup の主動作） / 検索 |
...
- 「Primary」がブレるとレイアウトが浮くので、新規ボタンは **その popup / form 内で何が主動作か** を考えて選ぶ。SavedInfo の編集 ✏️ もその popup の主動作なので Primary。
