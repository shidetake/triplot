...
| 文脈 | サイズの決め方 | 例 |
|---|---|---|
| **行内**（文字の隣） | **既定 16**（Lucide/shadcn の既定。本文 `text-sm`(14px)・`text-xs`(12px) の隣でもアイコンは 16 ＝ 業界標準の「アイコンは本文より少し大きい」）。比例ではなく段階的で、**コンパクト/高密度な行（テキスト ≤11px）でだけ 12 に落とす** | text-sm の行・戻りリンク → 16 / カレンダーや凡例の `text-[11px]` → 12 |
