...
| **入力・セレクト** | `h-9 px-3 text-sm rounded-md border-zinc-300` | focus は `focus:border-black focus:outline-none`。iOS のフォーカス時ズーム防止は **globals.css がモバイル幅で 16px を強制**するので入力は `text-sm` のままで可（個別に `md:` 等は不要） |
