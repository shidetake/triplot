...
- **メンバー色 = 円周上で“一番空いている方角”を選ぶ**。新メンバーには、「既に使われている
  ...
  （farthest-point 配置）。
- 実際の出力（図の ①②③ ＝挿入順）: ① = ★140° の正反対 **320°**、② = 残った半円の中点
  （**50° か 230°**・両方同距離でタイ）、③ = もう片方 → 結果 **{140, 320, 50, 230} で約90°間隔に
  均等**。片側に寄らない（増えるたび「残った一番広い隙間」に挿すため）。
- 実際の計算は SQL `pick_member_color`（毎回算出・プリセット無し）。
