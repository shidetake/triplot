...
- **× 閉じるボタン**（フォーム・ポップアップ右上）:
  `flex h-6 w-6 items-center justify-center rounded-full text-subtle-foreground transition hover:bg-zinc-100 hover:text-muted-foreground` ＋ `aria-label`/`title="閉じる"`
