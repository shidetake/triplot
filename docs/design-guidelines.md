...
| 文脈 | サイズの決め方 | 例 |
|---|---|---|
| **行内**（文字の隣） | **既定 16**（Lucide/shadcn の既定。本文 `text-sm`(14px) の隣でもアイコンは 16 ＝ 業界標準の「アイコン16・本文14」比）。本文より小さい文字（≤12px）の隣だけ縮めて合わせる | text-sm の行・戻りリンク → 16 / カレンダーの `text-[11px]` ブロック内 → 11 |
