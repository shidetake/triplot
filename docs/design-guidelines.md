...
- **画面に応じて見た目を変えるときは「何を本質的に切り替えたいか」を見極め、それを決める軸で判定する**。
  ...
  - **使える幅で決まること**（要素が画面に対して大きすぎる等）→ **ビューポート幅**で判定。
    例: 大きい入力フォームは狭い画面で**ボトムシート**（`vaul`＝ドラッグで閉じる・下からせり上がり、上に元画面が
    薄暗く残る＝どこから開いたかが分かる。全画面だと文脈が消えるため）、広い画面ではポップアップ（`FormPopover` の `fullScreenOnNarrow`）。
    Base UI にシートが無いので「ライブラリを使う」段（Google マップ等と同じ世の中標準の挙動）。
