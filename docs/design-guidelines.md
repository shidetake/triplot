...
| **hover 背景**（メニュー項目・リスト行・アイコンボタン） | `hover:bg-accent` | shadcn のメニュー hover と同じトークン（ライトで oklch(0.97)≒zinc-100）。`hover:bg-zinc-50/100` のベタ書きはしない。選択中の行の背景も `bg-accent` |
| **入れ子の hover**（行 hover の上に乗るボタン等） | `hover:bg-foreground/10` | 半透明オーバーレイなので下の色に重なるたび自動で一段濃くなる（Material の state layer 方式）。何段ネストしても同じクラスで済み、ダークモードも foreground 反転で自動。色付きコントロール（いいね済み♥等）はその色で `hover:bg-rose-500/10` のように |
...
インラインのメッセージボックスは色で深刻度を表す:

| 種類 | 形 | 例 |
|---|---|---|
| **エラー**（操作が失敗した・進めない） | `rounded-md bg-red-50 p-3 text-sm text-red-700`（密な場所では `p-2 text-xs`） | フォーム送信失敗 |
| **警告・注意**（進めるが知っておくべき） | `rounded-md bg-amber-50 p-3 text-sm text-amber-800` | 上限接近・非推奨状態 |
...
- **非インタラクティブな情報ハイライト**（例: カレンダー上の旅行期間の帯 `bg-blue-50`、
  優先度「低」アイコンの `text-blue-500`〔Jira 慣例・色＋形状で色覚配慮〕）
- **テキストリンク・インラインのアクションテキスト**（`text-blue-600 hover:underline` 等）— web 慣例どおり

使ってはいけないのは:

- **選択・アクティブ状態** — 選択は primary トークン（「ボタンの配色」節）。メンバー色が任意 hue
  なので、blue(≒220°) のメンバーがいると選択色と識別できなくなる。hue 中立な primary（黒）が正しい。
- カレンダー上の予定ブロックの選択も同様: 色を blue で塗り替えず、**地色と同じ hue の濃い枠を
  太く**して示す（`eventHueSelectedBorder`、実質 2px）。Google Calendar と同方式で、選択した瞬間に
  誰の予定か分からなくなるのを防ぐ。地色と無関係な色（黒・blue）の枠は乗せない。
- **例外**: `place-icon-picker` の「追加候補 = blue / 削除候補 = red」ペア。diff 的なプレビュー配色で、
  世の中の慣例は追加 = green だが、triplot は green(140°) を「確定/全員」に予約済みのため blue を使う。
...
- **× 閉じるボタン**（フォーム・ポップアップ右上）:
  `flex h-6 w-6 items-center justify-center rounded-full text-subtle-foreground transition hover:bg-accent hover:text-muted-foreground` ＋ `aria-label`/`title="閉じる"`
