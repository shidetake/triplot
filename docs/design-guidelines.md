...
member 色の地のブロック・hue 地色・primary 塗りのボタン等は「単一の文字色」が無いので、要素を丸ごと薄くするしかない。色トークンは当てられない。
...
判断基準: **「ずっと薄いインク」なら色トークン。「今この要素が dim/off の状態」なら opacity**（中立でも）。disabled が中立テキストでも opacity なのは後者だから。shadcn/Material と同じ。

### 非活性（disabled）

`disabled:opacity-50` で統一。`opacity-40` や `opacity-60` は使わない（状態の ON/OFF 切り替えなので opacity 側）。
