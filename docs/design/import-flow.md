...
割り当て先が決まらなかった下書きのうち、**移動か宿泊**のものは「まだ作っていない旅行が
ある」証拠になる。これを日付でまとめて、**旅行一覧に候補として出す**。押すと旅行作成
フォームが日程と名前を埋めた状態で開き、作れば普通の旅行になって、そのまとまりのメールが
そこへ割り当てられる。以降の確定は通常フローと同じ。

仮予定・仮費用と**同じ3層**で、新しいテーブルもカラムも要らない。

| 層 | 予定・費用 | 旅行 |
|---|---|---|
| 真実の置き場 | `inbound_drafts` | 同じ（未割り当ての行） |
| 画面に出すもの | 下書きから毎回導出する疑似項目 | 未割り当ての下書きをまとめた候補 |
| 確定 | 費用・予定を作る | 旅行を作り、メールを割り当てる |

違いは粒度だけで、予定・費用が「下書き1行＝1項目」なのに対し、旅行は「複数メールの塊＝1件」
になる（下書き1行を複数まとめる前例は重なりのマージにある）。**未確定の状態を `trips` に
持たせない**のがこの設計の要点で、そうしないと精算・メンバー・費用カテゴリの seed・
旅行ピッカー・各種バッジが「仮のときは？」を知る羽目になる。

**まとまりの決め方**は「期間の差が1日以内」という辺で結んだ**連結成分**。

- 各メールは下書きから期間 `[開始, 終了]` を持つ（宿泊は泊数ぶんの区間になる）。点ではなく
  区間で持つのは、長期滞在の最終日に隣接する便を拾うため。
- そのため**取り込み順に依存しない**。往路(1/1)と復路(1/3)は2日空くので最初は別々でも、
  後から 1/2〜1/3 の宿泊が入れば両方と繋がって 1/1〜1/3 の1件になる。どの順で届いても同じ。
- 閾値を1日と狭くしているのは**外し方が非対称**だから。分けすぎた時は片方を作って
  もう片方を割り当て直せば済むが、まとめすぎた時に予定・費用を別の旅行へ移す手段は無い。
  迷ったら分ける。

**旅行名**は場所から取る（宿泊の場所 → 到着地の順に重みを付けた最頻値。市 → 地域の順）。
宿泊を優先するのは、到着地だけだと乗り継ぎの経由地を拾うため。場所が解決できていなければ
名前は付けず、作成フォームで人が入れる。
...
テキスト系 content-type 以外は破棄。加えて **detailUrl が配信解除/購読設定リンクらしい
場合はそもそも fetch しない**（`isLikelyUnsubscribeUrl`。`unsubscribe`/`opt-out` 等の
キーワードを path/query に含むか。URL の綴りに公式標準は無いが、規制上の要請で主要
ESP がほぼ例外なく使う語＝事実上のデファクト）。これにより、ユーザのメール購読を
誤って操作するリスクを予防で潰す。スキップした場合も「疑いあり」として
`receipt_link_candidates` に記録し、admin 管理ページで可視化する。
...
| 観測 | 値 |
|---|---|
| ダッシュボードの残 Free Credit | **$4.71**（初期値は非公開） |
| 直近1時間の spend | **$0.15** |
| 直近1時間の Gateway リクエスト数 | **438**（大半はレート制限で弾かれた分。429 は課金されない） |
| 直近1時間に抽出**成功**したメール | **7通** |
| → **1通あたりの実費** | **約 $0.021**（$0.15 ÷ 7） |

- 最初の1〜2通だけ成功し、残り23通が `status=error`。全件に `next_retry_at` が入り打ち切りはゼロ
- 毎分の cron で再試行し続けて、**12分で2件成功**
- → 枠の枯渇ではなく**短期のレート制限に張り付いている**状態。枯渇なら1件も通らない

**2つの制限のうち、詰まらせているのはレート制限だけ。** Free Credit は $4.71 残っていて、
金額の枠には当たっていない。クレジットの購入が要るのは予算のためではなく**レート制限を
上げるため**、という切り分けになる。

**1通 $0.021 から見たコスト**（[billing.md](./billing.md) の月100件の枠と突き合わせる）:

| 想定 | 月あたり |
|---|---|
| 上限（100通/月）まで使うユーザー1人 | 約 $2.1 |
| 平均的な使い方（20通/月）のユーザー1人 | 約 $0.42 |
| 残 $4.71 で賄える人数 | 上限まで使う人なら**約2人**、平均的な使い方なら**約11人** |

無料枠のままでは公開に耐えない。1通あたりの単価はメールの長さで動く（この計測は明細の長い
レシート中心）ので、桁の目安として扱うこと。
...
- 1通のメールはたいてい「費用1件＋予定1件」を産む（同じ来店の別の見え方）。**確定も破棄も
  メール単位**で、片方を確定すると同じメールの残りも確定される（相方の費用・予定を下書きの
  既定値で実際に作る）。片方だけ確定すると相方が未確定のまま残り続けるため。例外は為替
  レートで、精算通貨と違ううえ同旅行に同通貨の履歴が無いと決めようがないので、その下書き
  だけ触らずフォームに送る。
