...
### 明細リンクの enrichment と学習

本文がスカスカで、品目・合計などの明細が「View your receipt」リンク先（Square/Clover 等の
レシート基盤）にしか無いメールがある。抽出前に、**許可ホスト**（`RECEIPT_LINK_HOSTS`）に一致する
リンクをサーバ側で fetch（SSRF ガード込み・[`fetchLink.ts`]）して本文に付加する＝enrichment。

許可ホストをどう増やすか（**学習**）。メールは処理後に消すので「保存メールを後で解析」はできない。
代わりに**抽出の瞬間に学習信号だけ抜く**:

1. 抽出スキーマに `detailUrl` を持たせ、LLM に「明細がリンク先にしか無いならその URL」を報告させる
   （既に呼ぶ LLM に相乗り＝追加コストなし。本文に実在する URL のみ採用＝幻覚除去）。
2. その `detailUrl` のホストが**未許可**なら、`receipt_link_candidates` に **host＋出現回数＋
   サンプル path**（メール本体やトークン付き URL は残さない）を upsert。削除ポリシーと両立。
3. 人が出現回数を見て、本物のレシート基盤を `RECEIPT_LINK_HOSTS`（コード定数）に**昇格**＝
   PR レビューでゲート。以後そのホストは 1 の enrichment で自動取得される。

> 昇格 UI は将来の admin 管理ページに載せる予定（当面は候補表を見て定数を編集）。
