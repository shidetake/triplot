...
flowchart TB
    ...
    T -.->|"乗継当日だけ、出発側/到着側の<br/>どちらかが参照される"| N
    D -.->|"乗継が無い日は<br/>常にこれが使われる"| N
...
- **ユーザーには見えない・変更するUIも無い。** その旅行で最初の予定/費用を作る瞬間に、
  ブラウザの現在TZで**一度だけ**自動セットされ、以後は据え置き。
...
sequenceDiagram
    ...
    participant RPC as create_event / create_expense
    participant DB as trips.default_timezone

    U->>RPC: 1件目の予定を作成(p_client_tz=ブラウザのTZ)
    RPC->>DB: NULLなら今回の値で埋める(以後は無視)
    Note over DB: 以後、乗継が無い日は<br/>ずっとこの値を使う
    U->>RPC: 2件目以降の予定を作成(p_client_tz は毎回送るが無視される)
```

**共有ライブラリ（`schedule.ts`）はブラウザTZを一切知らない。** サーバーで実行されると
「サーバーのTZ」になってしまい、ユーザーの実際の場所と無関係な値になるため。ブラウザTZが
顔を出すのは「新規作成フォームの初期値サジェスト」と「上記の一度きりの種まき」の2箇所だけ。
...
flowchart LR
    ...
    DTZ -->|"1. あれば優先"| SS
    BR -->|"2. 無ければこちら<br/>(かつ新規作成時の種にもなる)"| SS
```
