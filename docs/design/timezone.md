...
## 6. 費用の並び順も「参照のみ」に統一（occurred_at の廃止）

費用テーブルは以前 `occurred_at`（並び替え専用の絶対時刻キャッシュ）を持っていた。保存の
たびに `paid_at`（壁時計）をその時点で解決したTZで解釈し、一度だけ焼き込む方式。

```mermaid
flowchart LR
    subgraph OLD["旧: 保存時に確定（TZと矛盾する例外）"]
        O1["保存時にTZを解決"] --> O2["絶対時刻を計算して<br/>DBに焼き込む"]
        O2 --> O3["並び替えはこの<br/>キャッシュ値を使う"]
    end
```

これは「通常予定・費用は実TZ文字列を持たない」という上の方針と矛盾する例外だった
（旧設計—通常予定・費用がliteralなTZを保存していた頃—の名残）。乗継を後から作成/編集しても、
表示TZは自動追従するのに並び順だけ古いまま、という穴もあった。

**今は保存せず、表示のたびに毎回算出する**方式に統一した。

```mermaid
flowchart LR
    subgraph NEW["今: 表示のたびに毎回算出（一貫）"]
        N1["表示・並び替えのたびに<br/>TZを解決"] --> N2["壁時計の発生時刻と組み合わせて<br/>絶対時刻をその場で計算"]
        N2 --> N3["その値で並べる<br/>（保存はしない）"]
    end
```

トリップ単位の件数（数十〜数百）なのでDBの索引に頼る必要は無く、コストは実質ゼロ
（表示用のTZ解決は元々全件に対して行っていたので、それに相乗りするだけ）。副次効果として、
この絶対時刻計算にしか使っていなかったパラメータも呼び出しから丸ごと不要になった。
