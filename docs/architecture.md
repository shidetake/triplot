...
| **Google Cloud**（プロジェクト `935163799144`） | Google ログインの OAuth クライアント＋ Maps ＋ **People API** | web・iOS のクライアントは同じプロジェクト。**People API の有効化が要る**（下記） |

**People API を有効にしてあるのは、アバターのためだけ。** Google は写真を設定して
いないアカウントにも、頭文字入りの色付き丸を**画像として**返す。URL の形は本物の
写真と同じで区別がつかないため、有効化していないと「写真が無い人」を写真がある人
として扱ってしまい、旅行内でその人のメンバー色と表示名が出なくなる（アバターの
設計は [ui-guidelines.md](./ui-guidelines.md) の「定型部品」を参照）。

People API は写真ごとに `default` フラグを返し、これが生成画像かどうかを知る唯一の
正式な手段。必要なスコープ `userinfo.profile` はログイン時に既に付与されているので、
同意画面の変更も再同意も起きない。料金はかからない。

**判定に失敗した時は何もしない**（ログインを止めないため）。つまり有効化を忘れても
エラーは出ず、**アバターだけが静かにおかしくなる**。新しい Google Cloud プロジェクトに
移した時は、ここを忘れないこと。
...
- **mobile はさらに手前にシミュレータでの確認がある**（本番/確認用どちらのビルドも作らずローカルで動作確認。
  一番速いが実機固有の挙動は見れない）。3段階の使い分けと具体的なビルドコマンドは [`development.md`](./development.md) を参照。
