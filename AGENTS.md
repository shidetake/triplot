...
- **本番のデータは原則書き換えない・消さない。** スキーマの変更は migration を
  通す。調査のための読み取りはよい。自分が検証用に作ったものでも、本番では
  書き換え・削除をしない（検証は staging で行う）。どうしても本番のデータを
  書き換える必要がある時だけ、理由を説明してユーザーの許可を得てから行う。
