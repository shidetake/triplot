...
### 5. trip の磨き込み（小）
- [ ] 為替レート編集 UI（trip 詳細から per-expense のレートをまとめて見直す UI）
- [ ] メンバー色の手動変更 UI（現在は自動割り当てのみ）
- [ ] 費用カテゴリのカスタマイズ UI（`expense_categories` テーブルは trip ごとにカスタム対応済み。デフォルト 11 カテゴリは `key` で多言語対応済み。カスタムカテゴリは `key = NULL` で `name` をそのまま表示する）
...
直近: デザインガイドライン整備（placeholder・余白・HelpTip gap 等）が一巡。
次の機能着手候補: **#5 磨き込み**（レート編集・メンバー色）or **#13 ダークモード**。
