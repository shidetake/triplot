...
### 9. Apple ログイン追加
コード側は実装済み（LP/招待参加の Apple ボタン・`handle_new_user` の `full_name` 対応・
i18n）。残りは **Apple Developer Program 登録後の設定作業のみ**:
- [ ] Apple Developer Program 登録（審査 ~48h）
- [ ] App ID 作成（例 `app.triplot`）+ Sign in with Apple capability
- [ ] Services ID 作成（例 `app.triplot.web`）= client_id。
      Domains: `triplot.app` / Return URLs: `https://cjkiglocsrtnohoxcnfh.supabase.co/auth/v1/callback`
- [ ] Sign in with Apple 用 Key（.p8）作成 → Key ID / Team ID を控える
- [ ] client_secret JWT 生成（Supabase docs 手順。**有効期限 ≤6ヶ月**）
- [ ] Supabase Dashboard → Auth → Providers → Apple 有効化（Client IDs = Services ID、Secret = JWT）
...
- [ ] Apple client_secret（JWT）のローテーション: 有効期限が最大6ヶ月なので、Apple ログイン
      有効化後は失効前に再生成して Supabase Dashboard に貼り直す（#9 の設定完了後に期限をメモ）
