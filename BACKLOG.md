...
- [x] 他ページ（旅行詳細・設定・取り込み）のヘッダも同じ並びに → #14 の共有ヘッダー（`components/app-header.tsx`）で解消。
...
### 14. ランディングページ + URL/IA 再編 — [x] 骨組み実装済み（LP本体は別タスク）
...
1. [x] **ルート移設**：`app/page.tsx`（旅行一覧）→ `app/trips/page.tsx`。`/` は最小LP（ヒーロー＋ログイン/「アプリを開く」CTA）。未ログインで `/trips` 等→`/` へリダイレクト。
2. [x] **共有ヘッダー** `components/app-header.tsx` を作成し layout（`trips`/`settings`/`import`）で適用、各ページの自前トップ行を撤去。
3. [x] **戻り整理**：`/trips/[id]` の「← 旅行一覧」を `/trips` 向けに、設定/取り込みの「← ホーム」削除。
4. [x] **design-guidelines に「ナビ：ヘッダー/戻り/パン屑」節**を追記（単一の真実）。

これで #8 の「他ページのヘッダも同じ並びに揃える」は本項に吸収される。
- [ ] LP 本体（コピー/動画/スクショ）の制作は別タスクで後追い。
...
直近: #14 完了（骨組み）— `/` = 最小LP、`/trips` = 一覧、共有ヘッダー `app-header.tsx` を layout 適用、ナビ規約を design-guidelines に追記。
**残: LP 本体（コピー/動画/スクショ）の制作（別タスク）。**
